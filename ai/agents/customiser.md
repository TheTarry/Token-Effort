...
2. **Report** — Summarise the plan, including the proposed structure, files to create or modify, and any important design decisions. Ask the user to confirm they want to proceed. Any affirmative reply (e.g. "yes", "go ahead", "looks good") counts as confirmation. Once confirmed, invoke the `Customiser [Apply]` subagent with the full plan. The plan passed to the subagent must be self-contained and include: absolute file paths, the complete YAML frontmatter (shim files only), and the full body content for every file to be created or modified.
